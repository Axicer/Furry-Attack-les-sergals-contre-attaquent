...
public class CharacterAnimation {
	...
	public float[] getTextureCoordinate() {
		KeyFrame f = getBeforeKeyFrame();
		return new float[] {
				(f.diffX*((float)texture.width/(float)keyframes.length))/(float)texture.width, 		0f,
				((f.diffX+1)*((float)texture.width/(float)keyframes.length))/(float)texture.width, 	0f,
				((f.diffX+1)*((float)texture.width/(float)keyframes.length))/(float)texture.width, 	1f,
				(f.diffX*((float)texture.width/(float)keyframes.length))/(float)texture.width, 		1f
		};
	}
