...
public class GUISelector<T> extends GUIComponent{
	...
	public GUISelector(GUI gui, Vector3f pos, int width, int height, FontType type, Color textColor, GUIAlignement alignement, GUISelectorItem<T>... items) {
		this.items = items;
		...
		this.text = new GUIText(items[selectedIndex].getName(), new Vector3f(pos.x+alignement.getOffsetXfromCenter(width), pos.y+alignement.getOffsetYfromCenter(height),pos.z), 0f, type, textColor, ratio*0.2f, GUIAlignement.CENTER);
		...
		this.left = ComponentFactory.generateButton(gui, "<", ratio*0.2f, buttonsWidth, buttonsHeight, ratio*0.5f, leftPos, 0f, GUIAlignement.CENTER, new Runnable() {
			public void run() {
				getPreviousItem();
			}
		});
		...
		this.right = ComponentFactory.generateButton(gui, ">", ratio*0.2f, buttonsWidth, buttonsHeight, ratio*0.5f, rightPos, 0f, GUIAlignement.CENTER, new Runnable() {
			public void run() {
				getNextItem();
			}
		});
		this.bg = ComponentFactory.generateImage("/img/gui/background/gray-back-bg.png", width, height, new Vector3f(pos.x+alignement.getOffsetXfromCenter(width), pos.y+alignement.getOffsetYfromCenter(height),pos.z), GUIAlignement.CENTER);
	}
